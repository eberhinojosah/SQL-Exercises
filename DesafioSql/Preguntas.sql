...
SELECT
    ...
    SUM(qty*precio) AS ingreso_total,
    ROUND((100.0 * SUM(descuento) / NULLIF(SUM(qty*precio), 0)), 2) AS porcentaje_descuento
FROM dbo.ventas
...
SELECT
    ...
    ROUND((100.0 * SUM(descuento) / NULLIF(SUM(qty * v.precio), 0)), 2) AS porcentaje_descuento 
FROM dbo.ventas v
JOIN dbo.producto_detalle pd ON pd.id_producto = v.id_producto
GROUP BY pd.nombre_producto
